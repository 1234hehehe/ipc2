...
#define SENSOR_VERSION  "H20260122a"
...
// number  2b3   2b4  2b8   2b9   515   519   2d9
    {0x00, 0x00, 0x0, 0x01, 0x00, 0x30, 0x2c, 0x6a, 0},   // 1.000000
    {0x01, 0x20, 0x0, 0x01, 0x0b, 0x30, 0x2e, 0x6c, 14995},   // 1.171875
    {0x02, 0x01, 0x0, 0x01, 0x19, 0x30, 0x2c, 0x6a, 31177},   // 1.390625
    {0x03, 0x21, 0x0, 0x01, 0x2a, 0x30, 0x2e, 0x6c, 47704},   // 1.656250
    {0x04, 0x02, 0x0, 0x02, 0x00, 0x30, 0x2c, 0x6a, 65536},   // 2.000000
    {0x05, 0x22, 0x0, 0x02, 0x17, 0x30, 0x2e, 0x6c, 81159},   // 2.359375
    {0x06, 0x03, 0x0, 0x02, 0x33, 0x30, 0x2c, 0x6a, 97243},   // 2.796875
    {0x07, 0x23, 0x0, 0x03, 0x14, 0x30, 0x2e, 0x6c, 113240},   // 3.312500
    {0x08, 0x04, 0x0, 0x04, 0x00, 0x30, 0x2e, 0x6c, 131072},   // 4.000000
    {0x09, 0x24, 0x0, 0x04, 0x2f, 0x30, 0x2f, 0x6d, 147007},   // 4.734375
    {0x0a, 0x05, 0x0, 0x05, 0x26, 0x30, 0x30, 0x6e, 162779},   // 5.593750
    {0x0b, 0x25, 0x0, 0x06, 0x28, 0x30, 0x32, 0x70, 178776},   // 6.625000
    {0x0c, 0x0c, 0x0, 0x08, 0x00, 0x30, 0x33, 0x71, 196608},   // 8.000000
    {0x0d, 0x2c, 0x0, 0x09, 0x1e, 0x30, 0x36, 0x74, 212543},   // 9.468750
    {0x0e, 0x0d, 0x0, 0x0b, 0x0c, 0x30, 0x39, 0x77, 228315},   // 11.187500
    {0x0f, 0x2d, 0x0, 0x0d, 0x11, 0x30, 0x3c, 0x7a, 244423},   // 13.265625
    {0x10, 0x1c, 0x0, 0x10, 0x00, 0x30, 0x3f, 0x7d, 262144},   // 16.000000
    {0x11, 0x3c, 0x0, 0x12, 0x3d, 0x30, 0x42, 0x80, 278158},   // 18.953125
    {0x12, 0x5c, 0x0, 0x16, 0x19, 0x30, 0x47, 0x85, 293916},   // 22.390625
    {0x13, 0x7c, 0x0, 0x1a, 0x22, 0x30, 0x4b, 0x89, 309959},   // 26.531250
    {0x14, 0x9c, 0x0, 0x20, 0x00, 0x30, 0x4f, 0x8d, 327680},   // 32.000000
...
struct dgain_lut gc4653_dgain_lut[] = {
...
{
	/* Dgain table */
	struct dgain_lut *lut = gc4653_dgain_lut;
...
unsigned int gc4653_alloc_again(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_again)
{
	...
	isp_again = gc4653_again_lut[ag_index].gain;

	/*alloc dgain*/
	dg_index = alloc_dgain(isp_gain - isp_again);
	isp_dgain = gc4653_dgain_lut[dg_index].gain;
...
static int gc4653_setting_select(struct tx_isp_subdev *sd, int deboot)
{
	...
	case 0:
        ...
        gc4653_attr.again.max = Max_gain;
...
static int gc4653_set_analog_gain(struct tx_isp_subdev *sd, int value)
{
	...
	/*set again*/
	if(last_gain != gain){
		ret = gc4653_write(sd, 0x02b3, gc4653_again_lut[ag].reg2b3);
		ret = gc4653_write(sd, 0x02b4, gc4653_again_lut[ag].reg2b4);
		ret = gc4653_write(sd, 0x02b8, gc4653_again_lut[ag].reg2b8);
		ret = gc4653_write(sd, 0x02b9, gc4653_again_lut[ag].reg2b9);
		ret = gc4653_write(sd, 0x0515, gc4653_again_lut[ag].reg515);
		ret = gc4653_write(sd, 0x0519, gc4653_again_lut[ag].reg519);
		ret = gc4653_write(sd, 0x02d9, gc4653_again_lut[ag].reg2d9);
	}
	/* set dgain */
	if(last_gain != gain){
		ret += gc4653_write(sd, 0x020e, gc4653_dgain_lut[dg].dgain_b1);
		ret += gc4653_write(sd, 0x020f, gc4653_dgain_lut[dg].dgain_b2);
	}
...
static int gc4653_set_expo(struct tx_isp_subdev *sd, int value)
{
	...
	/*set integration time*/
	ret += gc4653_set_integration_time(sd, expo);
	/*gain*/
	ret += gc4653_set_analog_gain(sd, gain);

	return ret;
}


static int gc4653_set_dpc_blc(struct tx_isp_subdev *sd, unsigned int value)
{
	int ret = 0;
	...
	/*set dpc*/
	/*set global gain*/
	ret += gc4653_write(sd, 0x02b0, global_gain);

	return ret;
}

static int gc4653_set_logic(struct tx_isp_subdev *sd, int value)
{
	int ret = 0;

	struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
	static unsigned char temperature_flag = 2, frame_count = 10;
	unsigned char temperature_value = 0; 
	unsigned char logic_flag = 0;
	unsigned int ht_gain = Max_gain;

	ret = gc4653_read(sd, 0x0410, &temperature_value);
	if (ret < 0) return ret;
	...
	switch(temperature_flag){
		case 0: ht_gain = 327680, max_dgain = 65536, global_gain = 0x68; break;
		case 1: ht_gain = 327680, max_dgain = 65546, global_gain = 0x69; break;
		case 2: ht_gain = 327680, max_dgain = 65536, global_gain = 0x6a; break;
		case 3: ht_gain = 327680, max_dgain = 65536, global_gain = 0x6b; break;
		case 4: ht_gain = 327680, max_dgain = 131072, global_gain = 0x6c; break;
		case 5: ht_gain = 327680, max_dgain = 131072, global_gain = 0x70; break;
		case 6: ht_gain = 327680, max_dgain = 131072, global_gain = 0x74; break;
		case 7: ht_gain = 327680, max_dgain = 131072, global_gain = 0x78; break;
		case 8: ht_gain = 327680, max_dgain = 131072, global_gain = 0x80; break;
		default: ht_gain = 327680, max_dgain = 131072, global_gain = 0x88; break;
	}

	if(max_again != ht_gain){
		max_again = ht_gain;
	...
	ret += gc4653_set_dpc_blc(sd, temperature_flag);
...
static int gc4653_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case TX_ISP_EVENT_SENSOR_START:
		if(arg)
			ret = gc4653_set_logic(sd, *(int*)arg);
		break;
	default:
		break;
	}
