...
int accessAnalyzeSubscribeUriAndSend(ModuleHandle_T hModuleHandle,S32 nRecvID,void *pUriData,void *pCondition,cJSON_Struct **pOutParams)
{
...
void accessFreeSubscribeListNode(void * data)
{
...
int accessInsertSubscribeListNode(void * data)
{
	int iRet = -1;
	Access_SubscribeNode* node = (Access_SubscribeNode*)data;
...
S32 accessFindSubscribeListNode(void * a, void *b)
{
...
static S32 static_Access_Subscribe_fxn(ModuleHandle_T hModuleHandle,S32 nType /* 0-subscribe,1-unsubscribe,2-QueryEvent*/,S32 nRecvID,S8 *szSubscribeUri,cJSON_Struct **pQueryEventInfo,void *pUserData)
{
	...
	if(2 == nType)
	{
		...
		pNode = (Access_SubscribeNode*)Common_DList_Search(g_subscribeMgr.listdl, (void*)&node, accessFindSubscribeListNode);
		iRet = accessAnalyzeSubscribeUriAndSend(hModuleHandle,pNode->nRecvID,pNode->szValidUri,pNode->pCondition,pQueryEventInfo);
		return iRet;
	}
	else if(1 == nType)
	{
		node.nRecvID = nRecvID;
		node.szSubscribeUri = szSubscribeUri;
		return Common_DList_Delete(g_subscribeMgr.listdl, (void*)&node, accessFindSubscribeListNode);
	}
	else if(0 == nType)
	{
		...
		node.nRecvID = nRecvID;
		node.szSubscribeUri = szSubscribeUri;
		p = Common_DList_Search(g_subscribeMgr.listdl,(void*)&node,accessFindSubscribeListNode);
		if(p)
		{
		...
		accessInsertSubscribeListNode((void *)pNode);
		iRet = accessAnalyzeSubscribeUriAndSend(hModuleHandle,nRecvID,pUri,pJCondition,pQueryEventInfo);
		return iRet;
	}
	return 0;
}
	...
	while(1)
	{
		...
		if(g_iCfgChange){
			...
			for(i = 0;i < iCount;i++)
			{
				...
				if(iFlag)
				{
					accessAnalyzeSubscribeUriAndSend(hModuleHandle, pNode->nRecvID,pNode->szValidUri,pNode->pCondition,&pRoot);
...
int accessUpdateStoreItem(ModuleHandle_T hModuleHandle)
{
	S32 nRet = -1;
	...
	if(pAccessUsrCfgMgr->nUserCfgCount > 0)
	{
		...
		if (pArray)
		{
			...
			while(pUserCfg)
			{
				...
				if(Common_StrCmp(pUserCfg->szUserName, "(null)") == 0)
				{
                    pUserCfg= pUserCfg->pNext;
                    continue;
                }
...
int init_access_res(ModuleHandle_T hModuleHandle)
{
	...
	accessUpdateStoreItem(hModuleHandle);
	g_subscribeMgr.hModuleHandle = hModuleHandle;
	Common_Lock_Create(&g_subscribeMgr.hCfgLock, NULL);
	Common_DList_Init(&g_subscribeMgr.listdl,accessFreeSubscribeListNode);
