...
static int mygethostbyname(const char *hostname)
{
    unsigned int ipv4 = 0;
    ...
    else
    {
    	...
    	for (rp = result; rp != NULL; rp = rp->ai_next)
    	{
            ...
            {
    		    ipv4 = ntohl(((struct sockaddr_in*)rp->ai_addr)->sin_addr.s_addr);
                //LOGD("connecting ip=%d.%d.%d.%d\n", (ipv4>>24)&0xff, (ipv4>>16)&0xff, (ipv4>>8)&0xff, ipv4&0xff);
                break;
            }
    	}
...
int OS_CreateLocalIpcFd(const char *bindPath, int backlog)
{
    ...
    if(tcp) 
	{
        ...
        else 
        {
            ...
            {
                unsigned int ipv4 = mygethostbyname(host);
                //LOGD("Resolv host(%s) to %d.%d.%d.%d\n", host, (ipv4>>24)&0xff, (ipv4>>16)&0xff, (ipv4>>8)&0xff, ipv4&0xff);
                if (ipv4 == 0)
                {
                    tcp_ia = htonl(INADDR_ANY);
                }
...
int OS_FcgiConnect(char *bindPath)
{
	...
	*tp++ = 0;
    ...
    if(tcp == TRUE) {
        unsigned int ipv4 = 0;
    	if((ipv4 = mygethostbyname((*host ? host : "localhost"))) == 0) {
    	    fprintf(stderr, "Unknown host: %s\n", bindPath);
            LOGE("Unknown host: %s\n", bindPath);
    	    exit(1000);
	    }
    	sa.inetVariant.sin_family = AF_INET;
    	sa.inetVariant.sin_addr.s_addr = htonl(ipv4);
    	sa.inetVariant.sin_port = htons(port);
    	servLen = sizeof(sa.inetVariant);
    ...
    for (;;) 
	{
        ...
        for (;;) 
		{
            do 
			{
                ...
                socket = accept(listen_sock, (struct sockaddr *)&sa, (socklen_t *)&len);
            } while (socket < 0 
                     && errno == EINTR 
                     && ! fail_on_intr 
                     && ! shutdownPending);
...
int OS_IsFcgi(int sock)
{
    ...
    if (getpeername(sock, (struct sockaddr *)&sa, (socklen_t *)&len) != 0 && errno == ENOTCONN) {
        return TRUE;
    }
    else {
        return FALSE;
