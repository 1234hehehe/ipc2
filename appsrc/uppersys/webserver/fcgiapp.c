...
int FCGX_GetChar(FCGX_Stream *stream)
{
    if (stream->isClosed || ! stream->isReader)
        return EOF;
//	WEB_TMP("stream->rdNext:0x%x  stream->stop:0x%x\n", stream->rdNext, stream->stop);
	...
	//WEB_TMP("stream->stopUnget:0x%x  stream->rdNext:0x%x\n", stream->stopUnget, stream->rdNext);
    stream->stopUnget = stream->rdNext;
	//WEB_TMP("stream->rdNext:0x%x stream->stop:0x%x\n", stream->rdNext, stream->stop);
    if (stream->rdNext != stream->stop)
        return *stream->rdNext++;
 ...
 *----------------------------------------------------------------------
 */
static FCGI_Header * MakeHeader(FCGI_Header *header, 
        int type,
        int requestId,
        int contentLength,
        int paddingLength)
 ...
 *----------------------------------------------------------------------
 */
static FCGI_EndRequestBody *MakeEndRequestBody(FCGI_EndRequestBody *body, 
        int appStatus,
        int protocolStatus)
{
    if (body)
 ...
 *----------------------------------------------------------------------
 */
static FCGI_UnknownTypeBody *MakeUnknownTypeBody(FCGI_UnknownTypeBody *body, 
        int type)
{
    if (body)
    {
...
static void WriteCloseRecords(struct FCGX_Stream *stream)
{
    ...
    if(!(data->type == FCGI_STDERR
        ...
        FCGI_Header header;
        MakeHeader(&header, data->type, data->reqDataPtr->requestId, 0, 0);
    ...
    if(data->reqDataPtr->nWriters == 1) {
        ...
        MakeHeader(&endRequestRecord.header, FCGI_END_REQUEST, data->reqDataPtr->requestId, sizeof(endRequestRecord.body), 0);
        MakeEndRequestBody(&endRequestRecord.body, data->reqDataPtr->appStatus, FCGI_REQUEST_COMPLETE);
...
static void EmptyBuffProc(struct FCGX_Stream *stream, int doClose)
{
    ...
    if(!data->rawWrite) {
        ...
        if(cLen > 0) {
            ...
            MakeHeader((FCGI_Header *)data->buff, data->type, data->reqDataPtr->requestId, cLen, eLen - cLen);
        } else {
            stream->wrNext = data->buff;
	}
    }
...
static int ProcessManagementRecord(int type, FCGX_Stream *stream)
{
    ...
    if(type == FCGI_GET_VALUES) {
        ...
        FCGI_Header * pointer = (FCGI_Header *)response;
        MakeHeader(pointer, FCGI_GET_VALUES_RESULT, FCGI_NULL_REQUEST_ID, len, paddedLen - len);
        FreeParams(&paramsPtr);
    } else {
        paddedLen = len = sizeof(FCGI_UnknownTypeBody);
        FCGI_UnknownTypeRecord * pointer = (FCGI_UnknownTypeRecord *)response;
        MakeHeader(&pointer->header, FCGI_UNKNOWN_TYPE, FCGI_NULL_REQUEST_ID, len, 0);
        MakeUnknownTypeBody(&pointer->body, type);
    }
...
static int ProcessBeginRecord(int requestId, FCGX_Stream *stream)
{
    ...
    if(data->reqDataPtr->isBeginProcessed) 
	{
         ...
         */
        FCGI_EndRequestRecord endRequestRecord;
        MakeHeader(&endRequestRecord.header, FCGI_END_REQUEST, requestId, sizeof(endRequestRecord.body), 0);
        MakeEndRequestBody(&endRequestRecord.body, 0, FCGI_CANT_MPX_CONN);
...
#if 0
	WEB_TMP("header.version:0x%x\n", header.version);
	WEB_TMP("header.type:0x%x\n", header.type);
	WEB_TMP("header.requestIdB1:0x%x\n", header.requestIdB1);
	WEB_TMP("header.requestIdB0:0x%x\n", header.requestIdB0);
	WEB_TMP("header.contentLengthB1:0x%x\n", header.contentLengthB1);
	WEB_TMP("header.contentLengthB0:0x%x\n", header.contentLengthB0);
	WEB_TMP("header.paddingLength:0x%x\n", header.paddingLength);
	WEB_TMP("header.reserved:0x%x\n", header.reserved);
#endif
