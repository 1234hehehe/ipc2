...
#define    TIMER_MALLOC(x)              Common_Malloc(x,sizeof(int),__func__,__LINE__)
#define    TIMER_FREE(x)                Common_Free(x,__func__,__LINE__)
#define    TIMER_STRDUP(x)              Common_StrDup(x,__func__,__LINE__)
...
typedef struct
{
    ...
    pthread_t timerTh;
    int tid;
    unsigned int maxId;                    //鏈?澶у畾鏃跺櫒ID
    timer_t *timers;                                //timer
    pthread_mutex_t mutex;
    MQ_HANDLE_H mqHandle;       //娑堟伅闃熷垪handle
} TIMER_CONTEXT_T;
...
int Timer_Init(unsigned int maxId, MQ_HANDLE_H mq_handle)
{
    ...
    s_timer_ct.timers = (timer_t *) TIMER_MALLOC(maxId * sizeof(timer_t));
