...
{
    thread_cond *cond = (thread_cond*) MEDIA_MALLOC(sizeof(thread_cond));
    if (cond == NULL)
    {
...
{
    thread_worker *thr = (thread_worker*) MEDIA_MALLOC(sizeof(thread_worker));
...
static void worker_run(pthread_pool_t *thr_pool, thread_worker *thr, pthread_job_t *job)
{
...
static int job_deliver(pthread_pool_t *thr_pool, thread_worker *thr, pthread_job_t *job)
{
...
static void job_add(pthread_pool_t *thr_pool, pthread_job_t *job)
{
    ...
    if (thr_pool->count < thr_pool->parallelism)
    {
        ...
        pthread_pool_assert(pthread_create(&id, &thr_pool->attr, worker_thread, (void* ) thr_pool) == 0);
        thr_pool->count++;
...
pthread_pool_t *pthread_pool_create(const pthread_pool_attr_t *attr)
{
    ...
    thr_pool = (pthread_pool_t *) MEDIA_MALLOC(sizeof(*thr_pool));
    if (thr_pool == NULL)
    {
      LOGE("alloc mem failed\n");
        return NULL;
    ...
    pthread_pool_assert(pthread_attr_setdetachstate(&thr_pool->attr, PTHREAD_CREATE_DETACHED) == 0);
...
{
    if (thr_pool == NULL || thr_pool->valid != PTHREAD_POOL_VALID || poller_fn == NULL)
    {
       LOGE("%s\n",strerror(EINVAL));
        return -EINVAL;
    }
...
int pthread_pool_start_poller(pthread_pool_t *thr_pool)
{
    ...
    pthread_pool_assert(pthread_create(&id, &thr_pool->attr, poller_thread, (void* ) thr_pool) == 0);
...
pthread_job_t *pthread_pool_alloc_job(void (*run_fn)(void*), void *run_arg, int fixed)
{
    pthread_job_t *job = (pthread_job_t*) MEDIA_MALLOC(sizeof(pthread_job_t));
    if (job == NULL)
    {
      LOGE("alloc mem failed\n");
        return NULL;
