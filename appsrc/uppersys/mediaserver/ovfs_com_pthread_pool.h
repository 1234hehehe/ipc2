#ifndef OVFS_COM_PTHREAD_POOL_INCLUDE_H
#define OVFS_COM_PTHREAD_POOL_INCLUDE_H
...
typedef struct pthread_job_t
{
    struct pthread_job_t *next;
    void (*worker_fn)(void *arg); /* user function              */
    void *worker_arg; /* user function's arg        */
    int fixed; /* if can be freed ?          */
    long long int start; /* start time stamp           */
} pthread_job_t;

//typedef struct pthread_job_t pthread_job_t;
 ...
 * @param fixed {int}
 * @return {pthread_job_t*} 返回创建的工作任务
 */
pthread_job_t *pthread_pool_alloc_job(void (*run_fn)(void*),
        void *run_arg, int fixed);

/**
 * 释放由 pthread_pool_alloc_job 创建的工作任务
 * @param job {pthread_job_t*}
 */
 ...
 * 线程池对象结构类型定义
 */
typedef struct pthread_pool_t pthread_pool_t;
...
#define PTHREAD_POOL_DEF_THREADS   100  /**< 缺省最大值为 100 个线程 */
    int idle_timeout; /**< 工作线程空闲超时时间(秒) */
#define PTHREAD_POOL_DEF_IDLE      0    /**< 缺省空间超时时间为 0 秒 */
 ...
 * @param stack_size {int} 工作线程栈大小
 * @return {pthread_pool_t*}, 如果不为空则表示成功，否则失败
 */
pthread_pool_t *thread_pool_create(int threads_limit, int idle_timeout, int stack_size);

/**
 * 创建一个线程池对象
 * @param attr {pthread_pool_attr_t*} 线程池创建时的属性，如果该参数为空，
 *  则采用默认参数: PTHREAD_POOL_DEF_XXX
 * @return {pthread_pool_t*}, 如果不为空则表示成功，否则失败
 */
 ...
 * @param thr_pool {pthread_pool_t*} 线程池对象，不能为空
 * @param timewait_sec {int} 休眠　的时间值, 建议将此值设置为 1--5 秒内
 * @return {int} 成功返回 0, 失败返回 -1
 */
int pthread_pool_set_timewait(pthread_pool_t *thr_pool, int timewait_sec);
 ...
 */
int pthread_pool_atinit(pthread_pool_t *thr_pool, int (*init_fn)(void *), void *init_arg);
 ...
 * @return {int} 0: OK; != 0: Error.
 */
int pthread_pool_atfree(pthread_pool_t *thr_pool, void (*free_fn)(void *), void *free_arg);
 ...
 * @return {int} 0: 成功; != 0: 失败
 */
int pthread_pool_add_one(pthread_pool_t *thr_pool, void (*run_fn)(void *), void *run_arg);

#define	pthread_pool_add	pthread_pool_add_one

/**
 * 向线程池添加一个任务
 * @param thr_pool {pthread_pool_t*} 线程池对象，不能为空
 * @param job {pthread_job_t*} 由 pthread_pool_alloc_job 创建的线程任务
 * @return {int} 0: 成功; != 0: 失败
 */
...
/**
 * 添加一个新任务, 前提是已经成功加锁, 即调用 pthread_pool_bat_add_begin 成功
 * @param thr_pool {pthread_pool_t*} 线程池对象，不能为空
 * @param run_fn {void (*)(void*)} 当有可用工作线程时所调用的回调处理函数
 * @param run_arg 回调函数 run_fn 所需要的回调参数
 * @return {int} 0: 成功; != 0: 失败
 */
int pthread_pool_bat_add_one(pthread_pool_t *thr_pool, void (*run_fn)(void *), void *run_arg);
 ...
 * pthread_pool_create 后通过此函数设置调度函数，然后再
 * 调用 pthread_pool_start_poller 启动后台调度函数，该后台
 * 调度函数会不断地调用 poller_fn (即用户的回调函数)，用户可以
 * 在回调函数里调用 pthread_pool_add 将新任务添加进线程池中
 * @param thr_pool {pthread_pool_t*} 线程池对象，不能为空
 * @param poller_fn {int (*)(void*)} 循环检测任务队列的回调函数
 * @param poller_arg {void*} poller_fn 所需要的参数
 * @return {int} 0: 成功; != 0: 失败
 */
int pthread_pool_set_poller(pthread_pool_t *thr_pool, int (*poller_fn)(void *), void *poller_arg);
...
/**
 * 以批处理方式进行任务的分发, 其内部其实是调用了 pthread_pool_add_one()
 * @return 0: OK; -1: err
 */
int pthread_pool_add_dispatch(void *dispatch_arg, void (*run_fn)(void *), void *run_arg);

/**
 * 以单个添加的方式进行任务的分发
 * @return 0: OK; -1: err
 * 注：worker_fn 中的第二个参数为WORKER_ATTR结构指针，由线程池的某个
 *     工作线程维护，该结构指针中的成员变量 init_data 为用户的赋值传送变量，
 *     如：数据库连接对象等。
 */
int pthread_pool_dispatch(void *dispatch_arg, void (*run_fn)(void *), void *run_arg);
 ...
 * 设置线程任务调度超时警告的时间(毫秒)
 * @param thr_pool {pthread_pool_t*} 线程池对象，不能为空
 * @param n {long long int} 当该值 > 0 时，如果线程任务的调度时间超过此值则会记录警告日志(毫秒)
 * @return 0: OK; -1: err
 */
 ...
 * @param thr_pool {pthread_pool_t*} 线程池对象，不能为空
 * @param n {long long int} 当该值 > 0 时，子线程等待任务的超时等待基准时间(毫秒)
 * @return 0: OK; -1: err
 */
...
/**
 * 初始化线程池属性值
 * @param attr {pthread_pool_attr_t*}
 */
 ...
 */
void pthread_pool_attr_set_threads_limit(pthread_pool_attr_t *attr, int threads_limit);
 ...
 */
void pthread_pool_attr_set_idle_timeout(pthread_pool_attr_t *attr, int idle_timeout);
