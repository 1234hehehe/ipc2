...
{
  static int haveInitedBase64DecodeTable = 0;
  if (!haveInitedBase64DecodeTable) 
  {
    initBase64DecodeTable();
    haveInitedBase64DecodeTable = 1;
  }
  ...
  int ck = 0,ckLen,bFind;
  ckLen = strlen(base64Char);
     // in case "in" is not a multiple of 4 bytes (although it should be)
  for (int j = 0; j < jMax; j += 4) 
  {
    char inTmp[4], outTmp[4],c;
    for (int i = 0; i < 4; ++i) 
    {
        do{
            c = in[i+j];
            bFind = 0;
            for (ck = 0;ck < ckLen;ck++)
            {
                if (c == base64Char[ck])
                {
                    bFind = 1;
...
char* base64Encode(char const* origSigned, unsigned int origLength) {
