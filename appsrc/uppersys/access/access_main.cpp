...
S32 main(S32 argc,S8 *argv[])
{
	...
	nRet = Module_Init(&hModuleHandle,pConfig,NULL,static_Access_CallFunctions,NULL);
	LOGI("Module init %d\n",nRet);
	...
	nRet = request_channel_ability(hModuleHandle);
	if(0 != nRet)
	{
	...
	// 1. Load cfg
	nRet = access_loadCfg(hModuleHandle);
	if(nRet < 0)
	{
		do{
			nRet = access_CreateDefCfg(hModuleHandle);
			if(0 != nRet)
				Common_Sleep(1,0);
		}while(nRet != 0);
	}
	// 2. 初始化资源并注册订阅
	init_access_res(hModuleHandle);
	// 3.准备好，可以处理订阅及其他相关操作
	...
	if (pConfig != NULL)
	{
		...
		Common_Json_SetAttrValue(pConfig,-1,"Header/Uri",Common_Json_Type_String,"/",0,0);
	}
	nRet = Module_CallFunctions(hModuleHandle,pConfig,&pOutParams,3000);
	printf("nRet = %d :\n");
	{
		char *pOutString = NULL;
		pOutString = Common_Json_Print(pOutParams,NULL);
		if (pOutString != NULL)
		{
			printf("%s",pOutString);
		}
	...
	while(1)
	{
		...
		Common_Sleep(1,0);
		//Module_QueryEvent(hModuleHandle,nSubHandle,&pOutParams,3000);
	}
#ifdef TEST_CHECKPATH
	{
		char *test="b[0]/c",*pNextString = NULL,*pString = NULL,*pCurr;
		S32 nIdx = -1;
		pCurr = test;
		printf("checkPath :\n");
		do 
		{
			nIdx = -1;
			pString = Module_JsonCheckPath(pCurr,&nIdx,&pNextString);

			printf("[%s] -> [%d]->[%s]\n",pString?pString:"nul",nIdx,pNextString?pNextString:"nul");
			pCurr = pNextString;
		} while (pString != NULL && pCurr != NULL);
	}
#endif
...
#ifdef TEST_JSONADD
	{
		cJSON_Struct *pRoot;
		char *pString;
		pRoot = Module_JsonAdd(NULL,"e[1]/f[0]",Module_JsonType_String,"haha",4,0);
		Module_JsonAdd(pRoot,"e[2]/f[0]",Module_JsonType_String,"hello",4,0);

		pString = Module_JsonPrint(pRoot,NULL);
		printf("Test JsonAdd： \n<%s>\n",pString);
		pRoot = Module_JsonParse(pString,NULL,NULL);
		pString = Module_JsonPrint(pRoot,NULL);
		printf("Test JsonAdd2： \n<%s>\n",pString);
	}
#endif
