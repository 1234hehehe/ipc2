...
*/
#define LOG_DEFAULT_LEVEL_CLOSE     "/tmp/logclose"
...
static void CheckLogLevel(LOG_CONTEXT_T *pContext)
{
    ...
    if (ret > 0)
    {
        if (atoi(lvBuf) >= COMMON_LOG_LV_BASE && atoi(lvBuf) <= COMMON_LOG_LV_CLOSE)
        {
...
S32 Common_Log_Create(Common_Log_T *phLog,S8 *tag, U32 defaultLevel)
{
    ...
    if (access(LOG_DEFAULT_LEVEL_BASE, F_OK) == 0)
        ...
        pContext->level = COMMON_LOG_LV_HIGH;
    else if (access(LOG_DEFAULT_LEVEL_CLOSE, F_OK) == 0)
        pContext->level = COMMON_LOG_LV_CLOSE;
...
#endif
    if (level <= pContext->level && pContext->level != COMMON_LOG_LV_CLOSE)
    {
        va_list ap;
        S8 *outBuffer;
